...
# ---------------------------------------------------------------------
        ...
        self.groups = groups
        self.use_bias = use_bias
        self.bias = torch.nn.Parameter(data=torch.ones(out_channels), requires_grad=True)
        self.bias_initializer = bias_initializer
        self.kernel_initializer = kernel_initializer
        ...
        for _ in range(next_dim_len):
            if self.num_dims-1 != max_dims:
                ...
                conv_layer = torch.nn.ConvTranspose2d(in_channels=self.in_channels,
                                            out_channels=self.out_channels,
                                            bias=self.use_bias,
                                            kernel_size=self.kernel_size[1:],
                                            stride=self.stride[1:],
    ...
    def forward(self, input):
        ...
        # Compute size of the output without stride
        size_ons = tuple([(size_i[x] - 1) - 2 * self.padding[x] + size_k[x] + 1 for x in range(len(size_i))])
        ...
        for i in range(size_k[0]):
            ...
            for j in range(size_i[0]):
                ...
                # Add results to this output frame
                out_frame = j * self.stride[0] - (i - size_k[0] // 2) + (size_i[0] - size_ons[0]) // 2 +1 #- 2*self.padding[0] - (1-size_k[0]%2)
